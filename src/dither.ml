...
let transform image =
  ...
  let _k =
    Image.foldi image ~init:() ~f:(fun ~x ~y _ (r, _, _) ->
      match Float.O.(Float.of_int r > Float.of_int img_max / 2.0) with
      | true ->
...
let%expect_test "dither_test" =
  ...
  let diff =
    Image.foldi img ~init:[] ~f:(fun ~x ~y diff (r, g, b) ->
      match
        (not (r = 0 && g = 0 && b = 0))
        || not (r = img_max && g = img_max && b = img_max)
      with
...
;;
(* Compare with ref let%expect_test "dither_test" = let img = Image.load_ppm
   ~filename:"../images/beach_portrait.ppm" |> transform in let ref_img =
   Image.load_ppm ~filename:"../images/reference-beach_portrait_dither.ppm"
   in let diff = Image.foldi img ~init:[] ~f:(fun ~x ~y diff (r, g, b) -> let
   r1, g1, b1 = Image.get ref_img ~x ~y in match r = r1 && b = b1 && g = g1
   with | false -> (x, y, (r, g, b), (r1, g1, b1)) :: diff | true -> diff) in
   if List.length diff > 0 then ( print_s [%message "x-cord, y-cord, actual,
